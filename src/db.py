...
def refresh():
    ...
    try:
        ...
        try:
            data = jwt.decode(
                refreshToken, config["REFRESH_SECRET"], algorithms=["HS256"]
            )

            user = session.query(User).filter_by(id=data["user.id"]).first()
...
def createRefreshToken(user):
    ...
    token = jwt.encode(
        {"user.id": user.id, "exp": datetime.utcnow() + timedelta(days=7)},
